 ...
 * @LastEditTime: 2023-10-17 13:17:33
 */
package service
...
// 		return myerror.ErrTimeOut{Message: "编译超时"} //编译超时
// 	case err := <-done:
// 		if err != nil {
...
func (this GoCodeSandboxByNative) CompileFile(userCodePath string) error {
	...
	go func() {
		...
		if err != nil {
			if strings.Contains(err.Error(), "signal: killed") {
				msg := fmt.Sprintf("编译. 执行超时, err : %s", err.Error())
				done <- myerror.ErrTimeOut{Message: msg} //编译超时
			}
			done <- err
		}
		mylog.Log.Info("原生的编译输出: ", string(output))
		done <- nil
	}()
	...
	case <-time.After(commonservice.TIME_OUT):
		compileProcess.Process.Kill()
		msg := fmt.Sprintf("编译. 执行超时，限制为 %v 秒", commonservice.TIME_OUT)
		return myerror.ErrTimeOut{Message: msg} //编译超时
	case err := <-done:
		if err != nil {
...
func (this GoCodeSandboxByNative) RunFile(userCodePath string, inputList []string) ([]model.ExecResult, error) {
	...
	for i, input := range inputList {
		...
		if err != nil {
			if strings.Contains(err.Error(), "signal: killed") {
				msg := fmt.Sprintf("运行. 执行超时, err : %s", err.Error())
				return execResultList, myerror.ErrTimeOut{Message: msg} //运行超时
			}
